...
from topodoo_wechat_enterprises.models import wechat_enterprise
...
class WechatGLD(http.Controller):
    ...
    def get_sns_html(self, name):
        values = {"name": name}
        return request.render('topodoo_wechat_gld.get_sns_html', values)

    # 获得当前单据的日志SNS信息
    @http.route('/WechatGLD/get_sns', type='http', auth="public", csrf=False)
    def get_sns(self, gld_name):
        temp_list = []
        gld = request.env['gld'].sudo().search([('name', '=', gld_name)])
        message = request.env['mail.message'].sudo().search([('res_id', '=', gld.id), ('model', '=', 'gld')])
        if message:
    ...
    def get_add_gld_first_page(self, *args, **kw):
        ...
        temp_type = request.env['gld.template.type'].sudo().search([])
        return request.render('topodoo_wechat_gld.get_add_gld_first_page', {"user": user, "temp_type": temp_type})
    ...
    def get_add_gld_second_page(self, template_type, template, title, text, emergency, userid):
        ...
        return request.render('topodoo_wechat_gld.get_add_gld_second_page',
    ...
    def list_faqi(self, *args, **kw):
        data = request.env['gld'].sudo().search([('sponsor.user_id', '=', request.session['uid'])])
        user = request.env['res.users'].sudo().search([('id', '=', request.session['uid'])])
        return request.render('topodoo_wechat_gld.list_faqi', {"user": user, "data": data})

    # 待办工联单列表 页面
    @wechat_enterprise.wechat_login
    @http.route('/WechatGLD/list_daiban', type='http', auth="public", csrf=False)
    def list_daiban(self, *args, **kw):
        data = request.env['gld'].sudo().search(
            ['|',
             '&', ('copy_users_dy_ids.user_id', '=', request.session['uid']), ('state', '=', 'through'),
             '&', ('approver.user_id.id', '=', request.session['uid']), ('state', 'in', ('pending', 'pass'))])
        user = request.env['res.users'].sudo().search([('id', '=', request.session['uid'])])
        return request.render('topodoo_wechat_gld.list_daiban', {"user": user, "data": data})

    # 已办工联单列表 页面
    @wechat_enterprise.wechat_login
    @http.route('/WechatGLD/list_yiban', type='http', auth="public", csrf=False)
    def list_yiban(self, *args, **kw):
        data = request.env['gld'].sudo().search([('yi_approver_user_ids.user_id', '=', request.session['uid'])])
        user = request.env['res.users'].sudo().search([('id', '=', request.session['uid'])])
        return request.render('topodoo_wechat_gld.list_yiban', {"user": user, "data": data})
    ...
    def get_faqi_list(self, userid):
        ...
        obj = request.env['gld'].sudo().search([('create_uid', '=', int(userid))], limit=5)
    ...
    def add_faqi_list(self, userid, number):
        employee = request.env['hr.employee'].sudo().search([('user_id', '=', int(userid))])
        obj = request.env['gld'].sudo().search([('create_uid', '=', int(userid))], limit=5, offset=int(number))
        return self.onload_list(obj, userid)
    ...
    def save_opinion(self, wechat_gldid, opinion, check_state, userid):
        gld_bean = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        employee = request.env['hr.employee'].sudo().search([('user_id', '=', int(userid))])
        return request.env['gld.opinion'].sudo(userid).save_opinion_service_wechat(gld_bean.id, opinion,
                                                                                          check_state,
                                                                                          employee, 2)
    ...
    def get_daiban_list(self, userid):
        copy_users = request.env['gld'].sudo().search(
            [('copy_users_dy_ids.user_id', '=', int(userid)), ('state', '=', 'through')])
        obj = request.env['gld'].sudo().search(
            ['|',
    ...
    def xiangqing(self, name, qubie=None, userid=None):
        gld_obj = request.env['gld'].sudo().search([('name', '=', name.lstrip())])
        if_cs = False
        employee = request.env['hr.employee'].sudo().search([('user_id', '=', int(userid))])
        if gld_obj.state == 'through' and gld_obj.copy_users_dy_ids:
            for dy in gld_obj['copy_users_dy_ids']:
                if employee.id == dy.id:
                    ...
                    break
        if if_cs:
            request.env['gld'].sudo(userid).read_gld_service(gld_obj, 2, employee)
        message = request.env['mail.message'].sudo().search([('res_id', '=', gld_obj.id), ('model', '=', 'gld')])
        ...
        return request.render('topodoo_wechat_gld.xiangqing', {'gld_obj': gld_obj,
                                                                'userid': userid,
    ...
    def xiangqing_opinion(self, name, qubie=None):
        gld_obj = request.env['gld'].sudo().search([('name', '=', name.lstrip())])
        return request.render('topodoo_wechat_gld.xiangqing_opinion', {'gld_obj': gld_obj})
    ...
    def select_opinion(self, name, qubie):
        ...
        gld_obj = request.env['gld'].sudo().search([('name', '=', name)])
        return request.render('topodoo_wechat_gld.select_opinion',
    ...
    def get_opinion(self, name, shuzi):
        opinion = request.env['gld.opinion'].sudo().search([('gld_id', '=', name)])
        if opinion:
            opinion_list = []
    ...
    def get_gld_info(self, name):
        temp_type = request.env['gld'].sudo().search([('name', '=', name.lstrip())])
        copy_users = request.env['gld'].sudo().search([('copy_users.user_id', '=', request.session['uid'])])
        temp_type_list = []
        if temp_type:
    ...
    def get_temp(self, temp_type_id):
        temp = request.env['gld.template'].sudo().search([('temp_type', '=', int(temp_type_id))])
        temp_list = []
        for value in temp:
            temp_item = {}
            temp_item['id'] = value.id
        ...
        if save_type == 1:
            ...
            if gld:
                ...
                for appr in gld.approver:
                    ...
                    request.env['gld'].get_gld_agentid(appr, u'单号：' + gld.name, description, url)
            if id:
                return "1"  # True
            else:
                return "0"  # False
    ...
    def get_approver(self, user_id):
        approver = request.env['gld.service'].get_approver(user_id)
        employee_list = []
    ...
    def get_title(self, temp_id):
        approver = request.env['gld.template'].sudo().search([('id', '=', int(temp_id))])
        temp_list = []
    ...
    def gld_state_sent(self, wechat_gldid, userid):
        ...
        if gld_bean.approver_opinions:
            ...
            request.env['gld'].sudo().gld_state_sent_service(gld_bean, employee_obj)
            return "1"
        else:
            return "2"
    ...
    def gld_finish_to_pass(self, wechat_gldid):
        gld_bean = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        request.env['gld'].gld_finish_to_pass_service(gld_bean)
        return "1"
    ...
    def gld_state_cancel(self, wechat_gldid, userid):
        gld_bean = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        request.env['gld'].sudo(userid).gld_state_cancel_service(gld_bean)
        return "1"
    ...
    def read_gld_service(self, wechat_gldid, userid):
        gld_bean = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        request.env['gld'].sudo(userid).read_gld_service(gld_bean)
        return "1"
    ...
    def gld_state_draft(self, wechat_gldid, userid):
        gld_bean = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        if gld_bean:
            request.env['gld'].sudo(userid).gld_state_draft_service(gld_bean)
            return "1"
        else:
            return "2"
    ...
    def waiver(self, wechat_gldid, userid):
        ...
        employee = request.env['hr.employee'].sudo().search([('user_id', '=', int(userid))])
        if gld_bean:
            request.env['gld'].sudo(userid).waiver_service(gld_bean, userid, employee)
            return "1"
        else:
            return "2"
    ...
    def view_appr(self, no, name, userid):
        ...
        return request.render('topodoo_wechat_gld.view_appr', values)

    # 展示查看抄送人页面
    @http.route('/WechatGLD/select_appr_copy_user', type='http', auth="public", csrf=False)
    def select_appr_copy_user(self, no):
        values = {"no": no}
        gld = request.env['gld'].sudo().search([('name', '=', no)])
        return request.render('topodoo_wechat_gld.select_appr_copy_user',
    ...
    def get_copy_user(self, wechat_gldid):
        gld = request.env['gld'].sudo().search([('name', '=', wechat_gldid)])
        temp_list = []
        if gld:
    ...
    def add_approver_service(self, wechat_gldid, employee_id, name, userid):
        ...
        request.uid = userid
        if name == u"添加抄送人":
            result = request.env['gld.add.peoper.wizard'].sudo(userid).add_copy_peoper_service(gld_bean,
                                                                                                      employee, '',
                                                                                                      2)
        elif name == u"添加审批人":
            result = request.env['gld.add.approver.wizard'].sudo(userid).add_approver_service(gld_bean, employee,
                                                                                                     2)
        if result == "2":
            return "2"
        elif result == "3":
    ...
    def view_enclosure(self, name, *args, **kw):
        gld_obj = request.env['gld'].sudo().search(
            [('name', '=', name)])
        attachments = []
        if gld_obj:
        ...
        return http.request.render('topodoo_wechat_gld.select_enclosure',
    ...
    def enclosure_info(self, id, *args, **kwargs):
        attachments = http.request.env['ir.attachment'].sudo().search(
            [('id', '=', id)])
        return http.request.render('topodoo_wechat_gld.enclosure_info',
                                   {'attachments': attachments})
    ...
    @http.route(['/topodoo_wechat_gld/content',
                 '/topodoo_wechat_gld/content/<string:xmlid>',
                 '/topodoo_wechat_gld/content/<string:xmlid>/<string:filename>',
                 '/topodoo_wechat_gld/content/<int:id>',
                 '/topodoo_wechat_gld/content/<int:id>/<string:filename>',
                 '/topodoo_wechat_gld/content/<int:id>-<string:unique>',
                 '/topodoo_wechat_gld/content/<int:id>-<string:unique>/<string:filename>',
                 '/topodoo_wechat_gld/content/<string:model>/<int:id>/<string:field>',
                 '/topodoo_wechat_gld/content/<string:model>/<int:id>/<string:field>/<string:filename>'], type='http',
                auth="public")
