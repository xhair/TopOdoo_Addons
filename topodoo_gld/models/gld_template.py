...
class GldTemplate(models.Model):
    _name = "gld.template"
    _description = u'工联单模板'

    name = fields.Char(string=u"模板名称", required=True)
    temp_type = fields.Many2one('gld.template.type', string=u"模板分类", required=True, index=True)
    ...
    def create(self, vals):
        ...
        if len(obj) >= 1:
            ...
            return super(GldTemplate, self).create(vals)