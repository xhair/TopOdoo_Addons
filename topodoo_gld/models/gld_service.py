...
class GldService(models.Model):
    ...
    def delete_approver_by_uid(self, gld_id, open_id=None):
        ...
        if open_id:
            ...
            empid = self.env['gld.common'].get_login_user()['id']
        if empid:
            gld_bean = self.env['gld'].sudo().search([('id', '=', gld_id), ('approver', 'in', empid)])
            if gld_bean:
                vals = {'approver': [(3, empid)]}
                op_ids = self.env['gld.opinion'].sudo().search(
                ...
                if open_id == None:
                    gld_bean.message_post(
                        self.env['gld.common'].get_login_user()["name"] + u"确认,本工联单的审批,不在本人审批范围")
    ...
    def create_service(self, gld, pc_wechat=None):
        ...
        # SNS信息
        gld.message_post(self.env['gld.common'].get_login_user()["name"] + u"创建工联单")
        ...
        vals['user_id_gld'] = self.env["gld.common"].get_user_by_employee(gld['sponsor'].id)
        if pc_wechat != "wechat":
            # 给附件打水印
    ...
    def back_process_recode(self, other_name):
        ...
        """
        if other_name:
            gld_obj = self.env['gld'].search([('processe_name', '=', other_name)])
    ...
    def write_app_string(self, gld_id):
        ...
        :return:
        """
        gld_bean = self.env['gld'].search([('id', '=', gld_id)])
        ...
        if gld_id:
            employee_id = self.env['gld.common'].get_login_user()["id"]  # 获取员工的 id
