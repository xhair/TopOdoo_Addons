...
class AddCopyPeopleWizard(models.TransientModel):
    _name = 'gld.add.people.wizard'  # 对象名称
    ...
    def add_copy_people(self):
        ...
        if gld_data:
            self.add_copy_people_service(gld_data, self.copy_users, self.copy_declare, 1)
            # 修改工联单 的抄送人 待阅已阅字段
            write_copy_people_uids = self.write_copy_people_uids(gld_data.id)
            gld_data.write({'copy_users_dy_ids': write_copy_people_uids})

    def add_copy_people_service(self, gld, copy_users, copy_declare, shuzi):
        """
        添加抄送人
        :return:
        """
        ...
        if gld:
            if gld.state == u"through":
                for people in copy_users:
                    ...
                    description = u"%s抄送了一张标题为“%s”的工联单给您，请点击查看全文，马上处理！" % (self.env['gld.common'].get_login_user()["name"], gld.subject)
                    self.env['gld'].get_gld_agentid(people, u'单号：' + gld.name, description, url)
            ...
            for appr in wizard:
                ...
                exist_copy_users = False
                for gld_copy_users in gld.copy_users:
                    if appr.id in [gld_copy_users.id]:
                        exist_copy_users = True
                if shuzi == 1:
                    if exist_copy_users:
            ...
            if copy_declare:
                if not gld.copy_declare:
                    gld_vals['copy_declare'] = copy_declare + ';'
                else:
                    gld_vals['copy_declare'] = gld.copy_declare + copy_declare + ';'
            gld_vals['copy_users_dy_ids'] = apprs
            gld.write(gld_vals)
            gld.send_mess_gld(self.env['gld.common'].get_login_user()["name"] + u"添加新的抄送人:" + appr_names[:-1])

    def write_copy_people_uids(self, gld_id):
        ...
        for copy in copy_user_wizrd:  # 向导里应该是新增加的待阅人员，但是发起这个向导的人应该从待阅的名单中去除
            copy_users_dy_ids.append((4, int(copy.id)))
