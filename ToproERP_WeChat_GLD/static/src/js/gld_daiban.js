...
function add_data(){
    ...
    var list_daiban_body_div = $("#list_daiban_body_div");
    list_daiban_body_div.append("<p style='text-align: center;margin-top: 5px;font-style: italic;font-size: 14px;color: #0079BF' id='hint'>正在加载...</p>");
    add_list("/WechatGLD/add_daiban_list", "#list_daiban_body_div", 2, userid);
}
function add_list(url, adrs, shuzi, userid) {
    $.ajax({
        ...
        success: function (result) {
            ...
            number = number + 5;
            var list_daiban_body_div = document.getElementById("list_daiban_body_div");
            var hint = document.getElementById("hint");
            list_daiban_body_div.removeChild(hint);
